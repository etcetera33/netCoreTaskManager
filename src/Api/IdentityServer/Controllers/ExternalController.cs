...
namespace IdentityServer.Controllers
{
    ...
    public class ExternalController : Controller
    {
        ...
        private async Task<(UserDto user, string provider, string providerUserId, IEnumerable<Claim> claims)> FindUserFromExternalProvider(AuthenticateResult result, string subject)
        {
            ...
            var user = await _userService.GetByExternalId(subject);

            return (user, provider, providerUserId, claims);
        }

        private void ProcessLoginCallbackForOidc(AuthenticateResult externalResult, List<Claim> localClaims, AuthenticationProperties localSignInProps, UserDto user)
        {
            // additional claims
            localClaims.Add(new Claim(TaskManagerClaims.Name, user.Id.ToString()));
            localClaims.Add(new Claim(TaskManagerClaims.Role, user.Role));
